...
You can get a range of the number of columns by running the following two commands. If you are checking for any missing data (i.e., the existence of any line that is shorter than it should be), you should use the second command.
<pre><code>awk '{if( NF > max ) max = NF} END {print max}' filename</code></pre>
<pre><code>awk '{if( NF < min ) min = NF} END {print min}' filename</code></pre>
