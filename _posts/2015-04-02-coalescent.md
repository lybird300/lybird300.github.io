...
In principle a mutation model should contain a process describing insertions, deletions, and inversions, but these events occur so rarely that they normally can be ignored. For the infinite sites model, we can set up an overall rate to comprise both the rate of nucleotide mutations and the rate of insertions and deletions. For the finite sites model, however, usually only nucleotide substitutions are modeled.
